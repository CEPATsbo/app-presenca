...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  appId: "1:66122858261:web:7fa21f1805463b5c08331c"
    ...
    async function registrarVoluntario(voluntario, atividadesSelecionadas) {
        ...
        // Atualiza a ultima presença na ficha do voluntário, seja ele novo ou antigo
        const docRefVoluntario = doc(db, "voluntarios", voluntarioId);
        await updateDoc(docRefVoluntario, { ultimaPresenca: dataDeHoje, statusVoluntario: 'ativo' });
    ...
    if (btnRegistrar) {
        btnRegistrar.addEventListener('click', async () => {
            ...
            try {
                ...
                if (resultados.length > 0 && resultados[0].score < 0.35) { // Ajustando o limiar
                    const melhorResultado = resultados[0].item;
                    if (confirm(`Encontramos um nome parecido: "${melhorResultado.nome}". É você?\n\n(Clique 'OK' para confirmar ou 'Cancelar' para se registrar como um novo voluntário).`)) {
