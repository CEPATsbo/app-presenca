...
        
        // CORREÇÃO 2: Bloco mais seguro para contagem de votos, evitando erros.
        if (voto === 'aprovado') {
            const balanceteAtualizadoDoc = await balanceteRef.get();
            const balanceteAtualizadoData = balanceteAtualizadoDoc.data();

            let totalAprovacoes = 0;
            if (balanceteAtualizadoData && balanceteAtualizadoData.aprovacoes) {
                totalAprovacoes = Object.keys(balanceteAtualizadoData.aprovacoes).length;
            }

            if (totalAprovacoes >= NUMERO_DE_VOTOS_PARA_APROVAR) {
                await balanceteRef.update({ status: 'aprovado' });
                await logAuditoriaCollection.add({ 
                    acao: "BALANCETE_APROVADO_AUTO", 
                    autor: { nome: 'SISTEMA' }, 
                    timestamp: admin.firestore.FieldValue.serverTimestamp(), 
                    detalhes: { balanceteId, totalVotos: totalAprovacoes } 
                });
            }
        }
