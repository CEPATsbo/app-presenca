...
// --- MÓDULO EDUCACIONAL (CORAÇÃO DO SISTEMA - ATUALIZADO) ---
...
/**
 * ROBÔ 1 ATUALIZADO: Cadastra automaticamente as aulas da EAE ou Médiuns.
 */
exports.cadastrarAulasAutomaticamente = onDocumentCreated({ ...OPCOES_FUNCAO, document: 'cursos/{cursoId}' }, async (event) => {
    const snap = event.data;
    if (!snap) return null;
    const dadosCurso = snap.data();
    const cursoId = event.params.cursoId;
    const cursoNome = (dadosCurso.nome || "").toLowerCase();
    
    const curriculoRef = db.collection('cursos').doc(cursoId).collection('curriculo');
    const batch = db.batch();

    let aulasParaCadastrar = [];

    if (dadosCurso.isEAE === true) {
        aulasParaCadastrar = aulasEAE.map(a => ({ numero: a.numeroDaAula, titulo: a.titulo, ano: a.ano }));
    } else if (cursoNome.includes("médium") || cursoNome.includes("medium")) {
        aulasParaCadastrar = aulasMediuns.map(a => ({ numero: a.n, titulo: a.r ? `${a.t} (${a.r})` : a.t }));
    } else {
        return null; 
    }

    aulasParaCadastrar.forEach(aula => {
        const docRef = curriculoRef.doc();
        const docData = { numeroDaAula: aula.numero, titulo: aula.titulo };
        if (aula.ano) docData.ano = aula.ano;
...
/**
 * ROBÔ 2: Gera o cronograma inicial para novas turmas.
 */
exports.gerarCronogramaAutomaticamente = onDocumentCreated({ ...OPCOES_FUNCAO, document: 'turmas/{turmaId}' }, async (event) => {
    ...
    try {
        const aulasSnapshot = await db.collection('cursos').doc(cursoId).collection('curriculo').orderBy('numeroDaAula').get();
        ...
        aulasSnapshot.forEach(doc => {
            ...
            batch.set(ref, { ...doc.data(), gabaritoAulaId: doc.id, dataAgendada: admin.firestore.Timestamp.fromDate(dataAtual), status: 'agendada', isExtra: false });
            dataAtual.setDate(dataAtual.getDate() + 7);
        });
        return batch.commit();
    } catch (error) { console.error(`Erro cronograma ${turmaId}:`, error); }
});
